...
# Open and redirect to default upload webpage on the local page
@app.route('/')
def load_form():
...
def make_grayscale(input_image):
    ...
    decode_array_to_matrix= cv2.imdecode(image_array,cv2.IMREAD_UNCHANGED)
    print("Matrix image: ",decode_array_to_matrix)

    converted_gray_img = cv2.cvtColor(decode_array_to_matrix,cv2.COLOR_RGB2GRAY)
